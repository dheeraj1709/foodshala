...
class HomeModel extends CI_Model {
	...
	public function getPastOrders($userid){
		$this->load->database();
		$query = $this->db->query("SELECT * from orders inner join restaurant on restaurant.restaurant_unique_id = orders.restaurant_unique_id inner join items on items.item_unique_id = orders.item_unique_id where customer_unique_id = '$userid' and delivered = 'Y' ");
	...
	public function getOrderReferences($userid){
		$this->load->database();
		$query = $this->db->query("SELECT order_reference from orders where customer_unique_id = '$userid' and delivered = 'Y' group by order_reference");
		// print_r($query->result());
		return $query->result();
	}
